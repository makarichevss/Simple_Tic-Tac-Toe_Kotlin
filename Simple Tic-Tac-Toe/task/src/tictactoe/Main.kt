...
fun getCell(cells: MutableList<String>, char: Char): List<String> {
    ...
    while (true) {
        ...
        else if (numbers.size != 2 || numbers.any { it.toInt() !in range })
    ...
    cells[i1] = (if (i2 > 0) {
        cells[i1].substring(0, i2)
    } else {
        ""
    }) + char +
            if (i2 < cells[i1].lastIndex) {
                cells[i1].substring(i2 + 1)
            } else {
                ""
            }
...
fun printField(cells: List<String>) {
    val dashes = "-".repeat(9)

    println(dashes)
    cells.forEach { line ->
        print("| ")
        line.chunked(1).forEach { print("$it ") }
        println("|")
    }
    println(dashes)
}
